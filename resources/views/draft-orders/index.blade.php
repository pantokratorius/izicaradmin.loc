...
<div class="page-header">
  ...
  <div style="display: flex; gap: 10px; align-items: center;">
    <form method="GET" action="{{ route('draft-orders.index') }}">
      <input type="text" name="search" class="search-box"
             placeholder="Поиск по наименованию и номеру телефона"
             value="{{ request('search') }}">
    </form>
    <a href="{{ route('draft-orders.create', ['vehicle' => request('search_by_vehicle')]) }}" class="btn btn-add">+ Добавить</a>
  </div>
</div>
          ...
          <tr class="order-row" data-id="{{ $order->id }}">
            ...
            <td>
              <select onchange="openPrint(this, {{ $order->id }})" class="print-select">
                    ...
                    <option value="{{ route('draft-orders.print', $order->id) }}">Заказ 1</option>
                    <option value="{{ route('draft-orders.print2', $order->id) }}">Заказ 2</option>
                </select>
            </td>
        </tr>
        @empty
...
function toggleItems(orderId) {
  window.location.href = "{{ route('draft-orders.show', ':id') }}".replace(':id', orderId);
}

function openOrderModal(orderId) {
  window.location.href = "{{ route('draft-orders.edit', ':id') }}".replace(':id', orderId);
}
