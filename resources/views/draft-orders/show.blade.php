...
@section('title', 'Детали заказа №' . $draftOrder->id)
...
<div class="container py-4">
    <h1 class="mb-4">Заказ № {{ $draftOrder->order_number }}</h1>
    ...
    <div class="card mb-4">
        ...
        <div class="card-body" style="margin-left: 10px">
            <p><strong>Имя:</strong> 
                    {{ $draftOrder->client->first_name ?? $draftOrder->vehicle->client->first_name ??'—' }} {{ $draftOrder->client?->middle_name ?? $draftOrder->vehicle->client->middle_name ?? '' }} {{ $draftOrder->client?->last_name ?? $draftOrder->vehicle->client->last_name ?? '' }}
            </p>
            <p><strong>Email:</strong> {{ $draftOrder->client->email ?? $draftOrder->vehicle->client->email ?? '—' }}</p>
            <p><strong>Телефон:</strong> {{ $draftOrder->client->phone ?? $draftOrder->vehicle->client->phone ?? '—' }}</p>
        </div>
    </div>
<br>
    {{-- Информация о заказе --}}
    <div class="card mb-4">
        ...
        <div class="card-body" style="margin-left: 10px">
            <p><strong>Статус:</strong> {{ $status[$draftOrder->status] ?? '—' }}</p>
            <p><strong>Дата создания:</strong> {{ $draftOrder->created_at->format('d.m.Y H:i') }}</p>
        </div>
    </div>
<br>
    {{-- Позиции заказа --}}
    ...
    @if($draftOrder->client)&nbsp;&nbsp;&nbsp;<a href="{{route('clients.edit', $draftOrder->client->id)}}"> Перейти к клиенту</a>@endif 
        ...
        <div class="card-body p-0">
            <table class="table table-striped mb-0">
                ...
                <tbody>
                    @forelse($draftOrder->items as $item)
                    <tr id="item-row-{{ $item->id }}">
                        <td><input type="checkbox" class="item-checkbox" value="{{ $item->id }}"></td>
                        <td ondblclick='openItemModal({{ $draftOrder->id }}, @json($item))'>{{ $item->part_number }}</td>
                        <td ondblclick='openItemModal({{ $draftOrder->id }}, @json($item))'>{{ $item->part_make }}</td>
                        <td ondblclick='openItemModal({{ $draftOrder->id }}, @json($item))'>{{ $item->part_name }}</td>
                        <td ondblclick='openItemModal({{ $draftOrder->id }}, @json($item))'>{{ number_format($item->purchase_price, 2, ',', ' ') }}</td>
                        <td 
                            @if($item->sell_price > 0 )
                                style="background-color: #dcefff"
                            @endif
                        ondblclick='openItemModal({{ $draftOrder->id }}, @json($item))'>{{ $item->sell_price ? number_format($item->sell_price, 2, ',', ' ') : number_format($item->amount, 2, ',', ' ') }}</td>
                        <td ondblclick='openItemModal({{ $draftOrder->id }}, @json($item))'>{{ $item->quantity }}</td>
                        <td ondblclick='openItemModal({{ $draftOrder->id }}, @json($item))'>{{ number_format($item->summ, 2, ',', ' ') }}</td>
                        <td ondblclick='openItemModal({{ $draftOrder->id }}, @json($item))'>{{ $item->supplier }}</td>
                        ...
                        style="background-color: #dfffdc"
                            @endif
                            ondblclick='openItemModal({{ $draftOrder->id }}, @json($item))'>{{  $item->sell_price > 0 ? round( ($item->sell_price / $item->purchase_price - 1) * 100, 2) : ($item->margin ?? $globalMargin) }}
                        </td>
                        <td class="text-end" ondblclick='openItemModal({{ $draftOrder->id }}, @json($item))'>
                            {{ $item->comment }}
                        </td>
    ...
    <div style="margin: 30px 0 50px; display: flex; justify-content: space-between; align-items: flex-start">
        <div style="display: flex; flex-direction: column; justify-content: space-between; align-items: flex-start; height: 100px">
            ...
            <br>
            <a href="{{route('orders.edit', $draftOrder->id)}}"  class="btn btn-secondary">Перейти в заказ</a>
            <br>
            <div>
        ...
        <select onchange="openPrint(this, {{ $draftOrder->id }})" class="print-select">
                    <option value="">🖨️ Печать...</option>
                    <option value="{{ route('orders.print', $draftOrder->id) }}">Заказ 1</option>
                    <option value="{{ route('orders.print2', $draftOrder->id) }}">Заказ 2</option>
                </select>
    </div>
</div>
