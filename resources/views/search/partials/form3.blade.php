...
// 🔹 Создать кнопки для перехода к брендам
    ...
    if (brandNavDiv) brandNavDiv.remove(); // удалить старую панель, если была
      ...
      document.querySelector('#scrollTopBtn').style.bottom =   100 + 'px'
...
function addToSearch(item, row, quantity) {
  ...
  const stockData = {
    ...
    sell_price: item.price ?  numberFormat(item.price * (1 + percent / 100)).replace(/\s+/g, '').replace(',', '.') : 0,
...
.oem-row{background:#fff8c6 !important;}
