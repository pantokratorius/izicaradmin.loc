...
<!-- Лоадер -->
<div id="loader" style="display:none; margin:135px 0; text-align:center; position: absolute; left: 50%; top: 200px">
  ...
  if (sortMode === "delivery") {
    ...
    const borderStyle = idx === 0 ? "" : "border-top:0;border-bottom:0;";
     ...
     row.innerHTML = `
        ...
        <td class="purchase-price-col" title="${item.price ? parseFloat(item.price).toFixed(2) : "-"}">${item.price.toFixed(2)
    .replace('.', ',')
...
function addToOrderItems(item, row, quantity) {
  ...
  fetch("{{ route('orderitem_store_ajax') }}", {
    method: "POST",
    headers: {
    ...
    .then(() => {
      rowFlash(row, "#d4edda");
      showToast(`✅ Добавлено ${quantity} шт. в заказ`);
    })
    .catch(err => {
      console.error("Ошибка при добавлении в заказ:", err);
      rowFlash(row, "#ffe6e6");
      showToast("❌ Ошибка при добавлении в заказ", "error");
    });
}
...
function showQuantityPopup(item, row) {
  ...
  box.querySelector("#popupConfirm").addEventListener("click", () => {
    ...
    overlay.remove();
    addToOrderItems(item, row, qty); // 👈 function to send data to backend
  });
}
